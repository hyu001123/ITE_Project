...
bool ImageViewOnTimer(ITUWidget* widget, char* param)
{
    ...
    {
        bool ret = ituIconLoadJpegData(imageViewIcon, imageViewData, imageViewDataSize);
        if (!ret)
        {
            ITUScrollText* item;

            if (imageViewLoadPrev)
